...
final class Route{
	...
	public function pathinfoToArray(){
		$array = $tmp  = !empty ($this->url_query['path']) ?explode('/', $this->url_query['path']) :array();
		if (count($array) > 0) {
			if (isset($array[0])) {
				$this->route_url['app'] = $array[0];
				unset($array[0]);
			}
			if (isset($array[3])) {
				$this->route_url['controller'] = $array[3];
				unset($array[3]);
			}
			if (isset($array[4])) {
				$this->route_url['action'] = $array[4];
				unset($array[4]);
			}
			if(count($array) > 0){
				$this->route_url['params'] = $array;
